...
* NTP 服务器
  ...
  * `ntp1.aliyun.com`
* 点击右上角“同步”按钮测试
  * 如果失败，请确认是否能访问外网(e.g. 网关设置正确, 路由器添加了对应  VLAN 的回程路由)
* 勾选自动同步
* 保存
...
* 使用分机 IP 登录 Web 管理
  ...
  * 点击“提交”按钮(在手动设定时间的分割线上方）
  * 到电话机上查看时间是否正确

## 重启设备
* 右上角用户 > 重新启动

## 更换 IPPBX 和 IP Phone 的 IP 以及对应的 Vlanif
* 首先修改 IPPBX 的 WAN 的 IP(e.g. 192.168.6.12 -> 10.0.6.3)
* 登录旧电话机的 Web 管理后台设置新的 IP(e.g. 192.168.6.21 -> 10.0.6.51)，或者直接在电话机上设置新的 IP（修改IP和网关，速度要快，否则翻页被改回去修改失败）
* 修改交换机和路由器的 Vlanif 和静态路由
* 拔掉电话机的网线，重启 IP Phone(POE供电)
* 用新 IP 登录电话机的 Web 管理，在线路 > 重新提交，再次注册一次分机
* 用新的 IP 登录 IPPBX Web 管理 > PBX > 分机中，原分机号码的用户可能会显示故障，IP 为旧 IP
